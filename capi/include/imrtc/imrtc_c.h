 ...
 * Track 的媒体，订阅关系保留。
 *
 * **这道校验必须在这里做，因为服务端按设计不会拒绝你**（2026-09-07 查证）：
 * 协议 §2.4 规则 6 规定枚举收到集合外的值**必须兜底**（`max_layer` 的兜底值是
 * `"l"`）而不是报错——这条兜底正是 §10「新增枚举值不算破坏兼容」的前提，
 * 所以它是有意为之，不是服务端的疏漏（错误码表里的 1306 `layer_unavailable`
 * 已于同日标为**保留码、服务端不发**）。
 *
 * 后果是：层名写错一个字母，服务端照样回 `.ok`，你收不到任何错误回调，
 * 那条流被降到最低层——**画面只是糊，客户端这边一切正常**。
 * 所以这个同步返回的 `BAD_PARAMS` 是你唯一会收到的反馈。检查返回值。
 *
