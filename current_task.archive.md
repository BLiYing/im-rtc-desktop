...
## 2026-09-16（发布打包 + Demo 三档之前）：精简前全文

以下是 2026-09-16 加 SDK 发布打包（`scripts/package.sh`）与 Demo 三档 SDK 开关（`IMRTC_SDK_DIR`/
`scripts/demo.sh` 的 `IMRTC_SDK=source|local|public`）之前的活快照原文，供查历史用。
...
## 2026-09-19（握手超时收口时移出活快照）：09-18 深夜 publish_deferred 那段
