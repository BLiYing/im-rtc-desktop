...
>
> 四端统一的接入步骤与 API 速查见在线指南：启动 im-rtc-server 后访问 `/guide/desktop`；本文讲 C ABI 的规矩、渲染路径与构建平台，两者互补。
>
...
**层名写错，服务端按设计不会告诉你**：协议 §2.4 规则 6 规定枚举收到集合外的值
**必须兜底**（`max_layer` 兜到 `"l"`）而不是报错——这条兜底是 §10「新增枚举值
不算破坏兼容」的前提，所以它是有意为之。实测发 `"zzz"` 照样回 `.ok`。
后果是那条流被降到最低层，**画面只是糊，你这边一切正常**。

所以 C ABI **在自己这一侧挡了**：非法层名同步返回 `IMRTC_V1_ERR_BAD_PARAMS`，
不发上线路。**这是你唯一会收到的反馈，请检查返回值。**
