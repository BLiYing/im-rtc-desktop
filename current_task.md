...
5. **`engine/src/CallEngine.cpp` 只剩 11 行余量（589/600）——下一刀动它之前必须先拆。**
   这三刀往里加了 41 行（握手校验 +19、恢复触发 +9、闸门放闸与 send 返回值 +13）。
   拆的切法已经比较明显：`login/logout/updateToken` 那一段连接装配（约 90 行）
   可以整块搬去 `CallEngineSession.cpp`，它与状态机那部分几乎没有耦合。
   `demo/MainWindow.cpp` 512、`capi/src/imrtc_c.cpp` 537 也在预警线上。
