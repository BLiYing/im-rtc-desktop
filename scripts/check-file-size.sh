#!/usr/bin/env bash
# check-file-size.sh —— 防「上帝类」体检：单个 .cpp/.h 文件行数超阈值即失败（见 CONVENTIONS.md §2）。
#
#   全量（pre-commit / CI / test.sh 第 1 步）：  ./scripts/check-file-size.sh
#   指定文件：                                   ./scripts/check-file-size.sh a.cpp b.h
#   编辑期钩子单文件（静默，仅超限时 stderr+exit 2）： ./scripts/check-file-size.sh --hook path/to/x.cpp
...
#   ② 一个类多关注点 → 拆成多个类，或把实现按关注点分到多个 .cpp。
#   ③ 状态机膨胀   → 按状态族拆文件。
# 头文件里不写实现（除模板/constexpr）：内联实现会让编译期和体量一起膨胀。
#
...
is_skipped() {
  case "$1" in
    */tests/*|*_test.cpp|*_test.h|*.generated.cpp|*.generated.h|moc_*.cpp|ui_*.h|qrc_*.cpp) return 0 ;;
  esac
  return 1
}
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  mk 10 "$tmp/small.cpp"
  mk 999 "$tmp/big.cpp"
  mk 999 "$tmp/big_test.cpp"
  echo "== 门禁自检 =="
  chk 0 100 "小文件放行"            "$tmp/small.cpp"
  chk 1 100 "大文件拦截"            "$tmp/big.cpp"
  chk 0 100 "测试文件跳过"          "$tmp/big_test.cpp"
  chk 2 100 "--hook 超限返回 2"     --hook "$tmp/big.cpp"
  chk 0 100 "--hook 正常返回 0"     --hook "$tmp/small.cpp"
  echo ""
...
if [ "${1:-}" = "--hook" ]; then
  f="${2:-}"
  [ -n "$f" ] && [ -f "$f" ] || exit 0
  case "$f" in *.cpp|*.h|*.hpp) ;; *) exit 0 ;; esac
  is_skipped "$f" && exit 0
  lines=$(wc -l < "$f" | tr -d ' ')
  limit=$(limit_for "$f")
  if [ "$lines" -gt "$limit" ]; then
    echo "⚠ 体量超限：${f} ${lines} 行 > ${limit}。请拆分（抽协作对象 / 按关注点分到多个 .cpp），别放宽阈值。见 CONVENTIONS.md §2。" >&2
    exit 2
  fi
  exit 0
fi
...
for f in "${SRC[@]}"; do
  [ -f "$f" ] || continue
  case "$f" in *.cpp|*.h|*.hpp) ;; *) continue ;; esac
...
echo ""
if [ "$fail" -ne 0 ]; then
  echo "结果：✗ 有文件超预算——请拆分（抽协作对象 / 按关注点分到多个 .cpp），不要放宽阈值。见 CONVENTIONS.md §2。"
  exit 1
fi
